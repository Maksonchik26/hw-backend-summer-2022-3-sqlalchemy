...
class QuestionAddView(View, AuthRequiredMixin):
    ...
    async def post(self):
        ...
        if body["theme_id"] not in theme_ids:
            raise HTTPNotFound
        answers = [AnswerModel(title=answer["title"], is_correct=answer["is_correct"]) for answer in body["answers"]]
        question = await self.store.quizzes.create_question(body["title"], body["theme_id"], answers)
